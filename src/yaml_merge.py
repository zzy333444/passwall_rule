...
def merge_yaml_configs(
    other_template: Dict[str, Any], base_config: Dict[str, Any]
) -> Dict[str, Any]:
    """合并两个YAML配置"""
    ...
    def merge_dicts(
        wifi_dict: Dict[str, Any], base_dict: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        for key, wifi_value in wifi_dict.items():
            ...
            if isinstance(wifi_value, dict) and isinstance(base_value, dict):
                ...
                if key == "rules":
                    ...
                    existing_names = {
                        group.get("name")
                        for group in base_value
                        if isinstance(group, dict) and "name" in group
                    }
                    new_groups = [
                        group
                        for group in wifi_value
                    ...
                    new_proxies = [
                        proxy
                        for proxy in wifi_value
                        if (
                            isinstance(proxy, dict)
                            and proxy.get("name") not in existing_names
                        )
        ...
        if "proxy-groups" in minimal_config:
            for group in minimal_config["proxy-groups"]:
                if isinstance(group, dict) and "proxies" in group:
                    # 将proxies列表转换为单行格式
                    group["proxies"] = [
                        p.strip() if isinstance(p, str) else p for p in group["proxies"]
                    ]
